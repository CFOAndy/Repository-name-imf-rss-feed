...
def generate_rss(entries):
    ...
    for entry in entries:
        ...
        description.text = f"<![CDATA[{entry['content']}]]>"

    tree = ElementTree(rss)
    tree.write(os.path.join(OUTPUT_DIR, "media.xml"), encoding="utf-8", xml_declaration=True)
    ...
    for source_name, url in RSS_FEEDS.items():
        ...
        for entry in entries:
            ...
            if not is_duplicate(title, existing_titles):
                content = fetch_full_content(entry["link"])
                if not content:
